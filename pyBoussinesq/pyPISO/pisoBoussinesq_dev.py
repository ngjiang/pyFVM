...
def piso():
	...
	while time < final_time:
		...
		# Internal region following PDE
		sum_in_coef = (\
			in_coef['ddt(.)*dt']/time_step\
			+ in_coef['div(phi, .)'] \
			- in_coef['laplacian(.)']/Re
			).reshape(n_in_nodes*5)
		# Boundary region following BCs
		...
		while nCorr:
			...
			while nNonOrtho:
				...
				# Spd = Spd - int(Spd)
				print 'div(HbyA) = ', (AdV * Spd.reshape(n_nodes*n_nodes)).sum(), '/', (AdV * abs(Spd).reshape(n_nodes*n_nodes)).sum()
